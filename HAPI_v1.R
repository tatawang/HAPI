##setup work folder:
setwd("/XXXXXX/smoke_report_2014/")


## read the SMOKE output report inv files, county_scc in file name, all input files are already arranged format to csv
########################  read the inventory report  ##################################
### Here we only provide the example for nonpt and ptnonipm sector for 2011ek Emission Model platform data
################ for nonpt only can read multiple csv file one time ###################
txtpath = "/XXXXX/NIEHS_SBTEX_EMISSION/EMIS_REPORT_COUNTY_SCC/"
...
## select the Gulf Region state in the INV data
...
State_List6 = c(" Alabama       "," Georgia       "," Mississippi   "," Florida       "," Louisiana     "," Texas         ")

nonpt[which(nonpt$State %in% State_List1),] -> nonpt_6ST
ptnonipm[which(ptnonipm$State %in% State_List1),] -> ptnonipm_6ST

######################### Process 2A calculation for missing Area (gridding) emission   #########################
######## only run one sector one time. ##############
sector = nonpt_6ST
sector_mkup = nonpt
...
## select the output name for sector
name <- "nonpt_2014NEI_STEXABH_v2.X.csv"
write_missing_np_INV(name, MISSING_DF, No_STEX_misSCC, MSPC, MSPC_CODE )

######################### Process 2B calculation for missing point source #########################
######## only run one sector one time. ##############
...
name <- "ptnonipm_STEXAB_missing_2012v2.csv"
